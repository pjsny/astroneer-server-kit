...
const CheckRow: React.FC<{ check: Check }> = ({ check }) => {
  const { frame } = useAnimation({ interval: 80, isActive: check.status === 'checking' });
  ...
  const icon =
    ...
    check.status === 'checking' ? <Text color={colors.teal}>{spinnerFrames[frame % spinnerFrames.length]}</Text> :
...
const Group: React.FC<{ title: string; checks: Check[] }> = ({ title, checks }) => (
  <Box flexDirection="column" gap={1} marginBottom={1}>
    <Text color={colors.orange} bold>{title}</Text>
...
const Preflight: React.FC = () => {
  ...
  const [creds,   setCreds]   = useState<Check[]>([
    ...
    { label: 'Hetzner API token', status: 'pending' },
  ]);
    ...
    (async () => {
      ...
      if (env.HCLOUD_TOKEN) {
        ...
        update(setCreds, 1, {
          ...
          detail: res.ok ? undefined : 'token invalid — run: bun run setup',
        });
      } else {
      ...
      const SSH_KEY = env.SSH_KEY ?? `${process.env.HOME}/.ssh/astro-server`;
      ...
      // Use provider abstraction for volume + server checks
      update(setInfra, 2, { status: 'checking' });
      try {
        const volumeOk = await provider.checkVolume(env as Record<string, string>);
        update(setInfra, 2, {
      ...
      try {
        const serverRunning = await provider.checkServer(env as Record<string, string>);
        update(setInfra, 3, {
